...
export default async function ChineseHomePage() {
  const [rawEvents, rawAnnouncements, rawProducts, content, assets] = await Promise.all([
    getEvents(),
    getAnnouncements(3),
    getProducts(),
    getLandingPageContent("cn"),
  ...
  return (
    <>
      <SiteHeader locale="cn" content={content} logoSrc={logoImage} />
      <main lang="zh-CN">
          ...
          <div className="hero-content">
            <div className="hero-copy">
              ...
              <div className="hero-actions">
                <Link className="button button-coral" href="/cn/join">{content.heroPrimaryButton} <ArrowRight size={17} /></Link>
                <Link className="button button-ghost-light" href="/cn/events">{content.heroSecondaryButton}</Link>
              </div>
            ...
            <div className={`hero-visual ${heroImage ? "has-upload" : ""}`} aria-label={heroImage ? undefined : content.heroImageAlt}>
              ...
              <div className="hero-card hero-card-top"><span className="mini-icon"><ShieldCheck size={18} /></span><div><strong>{content.heroTopCardTitle}</strong><small>{content.heroTopCardText}</small></div></div>
              <div className="hero-card hero-card-bottom"><span className="pulse-dot" /><div><strong>{content.heroNextEventLabel}</strong><small>{nextEvent ? formatChineseEventDate(nextEvent.event_date) : content.heroNextEventFallback}</small></div></div>
        ...
        <section className="section section-intro" id="about">
          <div className="section-heading centered-heading"><p className="eyebrow">{content.introEyebrow}</p><h2>{content.introTitleLine}<br /><em>{content.introTitleAccent}</em></h2><p>{content.introDescription}</p><Link className="about-intro-link" href="/cn/about">{content.introAboutButton} <ArrowRight size={15} /></Link></div>
          <div className="benefit-grid">{benefits.map(({ icon: Icon, title, copy }, index) => <article className="benefit-card" key={`${index}-${title}`}><span className={`benefit-icon benefit-${index + 1}`}><Icon size={24} /></span><span className="card-number">0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div>
        </section>
        ...
        {nextEvent && <section className="section event-feature-section"><div className="event-feature">
          <div className={`event-art ${eventImage ? "has-upload" : ""}`}>
            {eventImage ? <img className="landing-cover-image" src={eventImage} alt={content.eventImageAlt} /> : <><div className="event-art-ring" /><div className="event-art-leaf"><Leaf size={130} /></div></>}
            <div className="event-art-copy"><small>{content.eventArtworkSmallText}</small><strong>{content.eventArtworkLargeText}</strong></div>
          </div>
          <div className="event-feature-copy"><p className="eyebrow">{content.eventEyebrow}</p><h2>{nextEvent.title}</h2><p>{nextEvent.description}</p><div className="event-meta-grid"><span><CalendarDays size={19} /><strong>{formatChineseEventDate(nextEvent.event_date, true)}</strong></span><span><Clock3 size={19} /><strong>{formatChineseEventTime(nextEvent.event_date)}</strong></span><span className="wide-meta"><MapPin size={19} /><strong>{nextEvent.location}</strong></span></div><Link className="button button-dark" href={nextEvent.id === "forum-2026-09-07" ? "/cn/events/ai-health-wealth-forum" : "/cn/member"}>{content.eventPrimaryButton} <ArrowRight size={17} /></Link><Link className="under-link" href="/cn/events">{content.eventSecondaryButton}</Link></div>
        </div></section>}
        ...
        <section className="notification-section">
          <div className="notification-copy"><span className="eyebrow eyebrow-light">{content.notificationEyebrow}</span><h2>{content.notificationTitleLine}<br /><em>{content.notificationTitleAccent}</em></h2><p>{content.notificationDescription}</p><NotificationButton publicKey={process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY} locale="cn" labels={{ idle: content.notificationButtonIdle, loading: content.notificationButtonLoading, enabled: content.notificationButtonEnabled, error: content.notificationError }} /></div>
          {notificationImage ? <div className="notification-upload"><img src={notificationImage} alt={content.notificationImageAlt} /></div> : <div className="phone-preview" aria-hidden="true"><div className="phone-speaker" /><div className="phone-time">{content.notificationPreviewTime}</div><div className="phone-leaf"><Leaf size={88} /></div><div className="phone-alert"><span className="brand-mark"><Leaf size={14} /></span><div><small>{content.notificationPreviewApp}</small><strong>{content.notificationPreviewTitle}</strong><p>{content.notificationPreviewMessage}</p></div></div></div>}
        </section>

        <section className="section membership-cta"><div className={`membership-card ${membershipImage ? "has-upload" : ""}`}>
          {membershipImage && <img className="membership-background-image" src={membershipImage} alt={content.membershipImageAlt} />}
          <div><p className="eyebrow eyebrow-light">{content.membershipEyebrow}</p><h2>{content.membershipTitleLine}<br /><em>{content.membershipTitleAccent}</em></h2><p>{content.membershipDescription}</p></div>
          <div className="membership-details"><p><Check size={17} /> {content.membershipBenefitOne}</p><p><Check size={17} /> {content.membershipBenefitTwo}</p><p><Check size={17} /> {content.membershipBenefitThree}</p><p><Check size={17} /> {content.membershipBenefitFour}</p><Link className="button button-coral button-full" href="/cn/join">{content.membershipButton} <ArrowRight size={17} /></Link><small>{content.membershipNote}</small></div>
        </div></section>
      </main>
      <SiteFooter locale="cn" content={content} logoSrc={logoImage} />
    </>
  );
}

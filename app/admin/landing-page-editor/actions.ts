...
function editorRedirect(kind: "saved" | "error", message: string): never {
  redirect(`/admin/landing-page-editor?${kind}=${encodeURIComponent(message)}`);
}
...
export async function updateLandingPageContentAction(formData: FormData) {
  ...
  for (const key of Object.keys(defaultLandingPageContent) as (keyof LandingPageContent)[]) {
    ...
    if (value.length > 2000) editorRedirect("error", `${key} is too long.`);
    content[key] = value;
  }

  const now = new Date().toISOString();
  await execute(
    `INSERT INTO landing_page_content (locale, content_json, updated_at) VALUES ('en', ?, ?)
     ON CONFLICT(locale) DO UPDATE SET content_json = excluded.content_json, updated_at = excluded.updated_at`,
    [JSON.stringify(content), now],
  );
  await writeAdminLog({
    activityType: "landing_page_content_updated",
    description: "Updated the English landing page text.",
  });
  revalidateLandingPage();
  editorRedirect("saved", "Landing page text saved");
}
...
export async function uploadLandingPageImageAction(formData: FormData) {
  ...
  if (!isAllowedLandingImageSlot(slot)) editorRedirect("error", "That image position is not valid.");
  if (!(image instanceof File) || image.size === 0) editorRedirect("error", "Choose an image to upload.");
  if (image.size > 4 * 1024 * 1024) editorRedirect("error", "Images must be 4 MB or smaller.");

  const mimeType = image.type.toLowerCase();
  const bytes = new Uint8Array(await image.arrayBuffer());
  if (!validImageBytes(bytes, mimeType)) editorRedirect("error", "Use a valid JPG, PNG, WebP, or GIF image.");
  ...
  revalidateLandingPage();
  editorRedirect("saved", "Landing page image uploaded");
}
...
export async function removeLandingPageImageAction(formData: FormData) {
  ...
  revalidateLandingPage();
  editorRedirect("saved", "Default artwork restored");
}
...
export async function updateLandingAnnouncementAction(formData: FormData) {
  ...
  if (!announcementId || title.length < 3 || message.length < 8 || !["community", "event", "promotion"].includes(kind)) {
    editorRedirect("error", "Please complete the announcement fields.");
  }
  await execute(
  ...
  editorRedirect("saved", "Announcement updated");
}
...
export async function updateLandingEventAction(formData: FormData) {
  ...
  if (!eventId || title.length < 3 || description.length < 8 || location.length < 3) {
    editorRedirect("error", "Please complete the featured event fields.");
  }
  await execute("UPDATE events SET title = ?, description = ?, location = ? WHERE id = ?", [title, description, location, eventId]);
  await writeAdminLog({
    eventId,
    activityType: "landing_event_updated",
    description: `Updated featured event: ${title}`,
  });
  revalidateLandingPage();
  revalidatePath("/events");
  revalidatePath("/cn/events");
  revalidatePath("/admin");
  editorRedirect("saved", "Featured event updated");
}
...
export async function updateLandingProductAction(formData: FormData) {
  ...
  if (!productId || name.length < 2 || description.length < 8 || !Number.isFinite(price) || price < 0 || !category) {
    editorRedirect("error", "Please complete the product fields.");
  }
  await execute(
  ...
  revalidateLandingPage();
  revalidatePath("/admin");
  editorRedirect("saved", "Product updated");
}
